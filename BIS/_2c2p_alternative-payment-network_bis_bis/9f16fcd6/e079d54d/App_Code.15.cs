#pragma checksum "D:\2C2P\Alternative-Payment-Network\BIS\BIS\App_Code\SSLPost.cs" "{ff1816ec-aa5e-4d10-87f7-6f4963833460}" "C55BF0AB9F4572C1D11867A47EDD654F93F8BE96"

#line 1 "D:\2C2P\Alternative-Payment-Network\BIS\BIS\App_Code\SSLPost.cs"
using log4net;
using System;
...
public class SSLPost
{
    ...
    public bool postXMLData(string postURL, string data2Post, out string responseData, out string errMsg)
    {
        ...
        try
        {
            ...
            bytes = System.Text.Encoding.ASCII.GetBytes(data2Post);//requestXml
    ...
    public bool postDateSolarHome(string apiUrl, string data2Post, out string responseData)
    {
        writeLog("$$$$$$$$$$$$$$$ POSTING TO DATA SOLAR $$$$$$$$$$$$$$$");
    ...
    public bool postDatautil(string postURL, out string responseData, out string errMsg)
    {
        ...
        try
        {
            ...
            //if (postURL != "")
            //{
            //    writeLog("data2post:" + postURL);
            //    StreamWriter postwriter = new StreamWriter(request.GetRequestStream());
            //    postwriter.Write(postURL);
            //    postwriter.Close();
            //}
            //else
            //{
    ...
    public bool post2EasyPoint(string backendUrl, string compressEncrypt, out string responseData, out string errMsg)
    {
        ...
        string encryptedMsg = pkcs7.encryptMessage(compressEncrypt, pkcs7.getPublicCert(ConfigurationManager.AppSettings["publicKeyPath"].ToString()));//Convert.ToBase64String(encodedEncryptedCms);
       ...
       // responseData = pkcs7.decryptMessage(responseData, pkcs7.getPrivateCert(ConfigurationManager.AppSettings["privateKeyPath"].ToString(), ConfigurationManager.AppSettings["privateKeyPWD"].ToString()));
        ...
        {
            string privatecer = ConfigurationManager.AppSettings["privateKeyPath"].ToString();
            string pass=ConfigurationManager.AppSettings["privateKeyPWD"].ToString();
